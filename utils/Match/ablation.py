# -*- coding: utf-8 -*-
"""
Contextual Graph Matching (CGM) matcher:
- 自注意(kNN) + 交叉注意 + dual-softmax(+dustbin) + 互一致 + 早停
- 粗到细 Top-K 候选门控（可选，默认开）
- 极线先验偏置（由第一轮RANSAC估计F/E，可选，默认开）
- 两段式(EM)姿态引导再匹配（默认开）
- 空间多样性采样用于姿态（提供函数，外部可用）
- 仍支持 ORB 后备（未传学习特征/Bank 时）

兼容你现有 experiment_match.py：
- 若传入 curr_feat_map / curr_bank_sim（以及缓存中的 prev_feat_map / prev_bank_sim），自动走学习特征路径
- 否则回退 ORB
"""
...
def _get_reweighter(device):
    ...
    if _REWEIGHT_HEAD is None:
        _REWEIGHT_HEAD = PairReweighter().to(device)
...
def match_two_stage_with_pose(
    ...
    *,               # 第一阶段（无先验）
    k_nn=16, tau=0.04, conf_thr_stage=(0.7, 0.5, 0.3),
...
class StatefulMatcher:
    ...
    def step(self,
             ...
             *,
             ...
             curr_bank_sim: Optional[torch.Tensor] = None,  # (Hf,Wf) or (H,W)
             use_reweighter: bool = True,
             ratio: Optional[float] = None):  # 新增：默认 True，支持禁用
